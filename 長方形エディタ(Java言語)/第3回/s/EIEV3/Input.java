...
class Input{
    ...
    boolean input(Board b){
        ...
        //createの呼び出し
        if(command==1){ 
            if(b.rects.size()==10)System.out.println("これ以上作成できません");
            else b.setRectangle(cmd.create(b));
        }
	...
	//displayBoardの呼び出し
	    ...
	    if(b.rects.isEmpty())System.out.println("要素がありません。");
	    else b.showRectangle();
	}

	//exitで終了
	else if(command==7)return false;
