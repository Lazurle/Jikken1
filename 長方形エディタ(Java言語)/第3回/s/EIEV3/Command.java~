...
//        System.out.println("create:呼び出されました。");	
	...
	try{
		...
		if(height<=1)System.out.println("2以上の数を入力してください");
	    }
	    while(width<=0){
		System.out.println("幅を入力してください。");
		width = new Integer(in.readLine());
		if(width<=1)System.out.println("2以上の数を入力してください");
	    }
	    //色の選択
	    while(true){
	...
	if(n!=0){
	    b.rects.remove(n-1);
	    System.out.println(b.rects.size()+"番目の長方形を削除しました。");
     }
    }
	...
	if(n!=0){
	    ...
	    Rectangle r=b.rects.get(n-1);
	    r.x=r.x+x;
	    r.y=r.y+y;
	    if(checkRect(b,r))b.rects.set(n-1,r);
	    else {
            System.out.println("ボードから長方形がはみ出しました。入力をやり直してください。");
            move(b);
        }
	...
	if(b.rects.isEmpty()){
	    ...
	    int n=0;
	    System.out.println("何番目の長方形を指定しますか？(1~"+b.rects.size()+"番目まで)");
	    while(true){
		try{
		...
		if(n>b.rects.size()||n==0){
		    System.out.println("存在しません。1~"+b.rects.size()+"番目を指定してください");
		}
		else return n;
	    }
	...
	else if(r.x<=1 || r.y<=1)return false;
	else return true;
    }
